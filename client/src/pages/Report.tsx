    ...
    const fetchAnalysis = async () => {
        try {
            ...
            if (res.data.success) {
                ...
                // 🔥 CACHE UI ALERTS 🔥 (Placed after processing logic to prevent spam during polling)
                if (res.data.source === 'cache') {
                    toast.success('Cache Hit! ⚡ SEO Report loaded from Redis.', {
                        id: 'report-cache-hit',
                        style: { backgroundColor: '#0F172A', color: '#fff', border: '1px solid #1E293B' }
                    });
                } else if (res.data.source === 'database') {
                    toast('Cache Miss! 🗄️ Fetching report from MongoDB...', {
                        id: 'report-cache-miss',
                        icon: '🔍',
                        style: { backgroundColor: '#0F172A', color: '#fff', border: '1px solid #1E293B' }
                    });
                }

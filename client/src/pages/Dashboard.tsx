        ...
        try {
            ...
            if (res.data.success) {
                ...
                if (res.data.source === 'cache') {
                    toast.success('Cache Hit! ⚡ Data served instantly from Redis.', {
                        id: 'dashboard-cache-hit', // prevents duplicate toasts
                        style: { backgroundColor: '#0F172A', color: '#fff', border: '1px solid #1E293B' }
                    });
                } else if (res.data.source === 'database') {
                    toast('Cache Miss! 🗄️ Loading fresh data from MongoDB...', {
                        icon: '🔍',
                        id: 'dashboard-cache-miss',
                        style: { backgroundColor: '#0F172A', color: '#fff', border: '1px solid #1E293B' }
                    });
                }
            }
